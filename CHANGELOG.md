...
- A "Upload images" action next to Import/Export on the Admin Products page: upload a batch of image files at once, and each is applied to whichever product's `image_path` (set via CSV/Excel import, which can reference an image that isn't on this server yet) matches its filename exactly. A notification reports how many matched and lists any that didn't.
- Every image upload in Admin (product image, this new bulk uploader, and the site logo) now keeps its original filename on disk instead of Filament's default random one, via `FileUpload::preserveFilenames()`. This is what makes filename-matched bulk upload possible, and also makes storage browsable by eye.
