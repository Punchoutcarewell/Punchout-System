...
|--------------------------------------------------------------------------
|
| The small same-origin JSON API described on CartController. Runs on the
| standard "web" group (cookies, CSRF) plus Punchout's session middleware:
| a request with no bound punchout session never reaches these actions.
|
*/

Route::middleware(['web', 'punchout.resolve-session', 'punchout.require-session'])
    ->prefix('storefront/cart')
    ->group(function (): void {
